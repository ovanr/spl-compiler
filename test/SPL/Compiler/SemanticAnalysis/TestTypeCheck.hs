...
type TypeCheckTest a = ((a, CoreType), Maybe (CoreType)) 
type TypeCheckTestEnv a = ((TypeEnv, a, CoreType), Maybe CoreType)
...
infixl 2 ~=
(~=) :: (a, CoreType) -> CoreType -> TypeCheckTest a
(a, t) ~= typ = ((a, t), Just typ)
...
infixl 2 .::
(.::) :: AST.ASTFunDecl -> CoreType -> TypeCheckTest AST.ASTFunDecl
f@(AST.ASTFunDecl{}) .:: typ = ((f, typ), Just typ)
...
(=::) (id, t) e = 
    ((AST.ASTVarDecl def (AST.ASTUnknownType def) (AST.ASTIdentifier def id) e, t), Just t)
...
executeCoreTests tests evaluator =
    forM_ tests $ \((gamma, a, initialTyp), expected) -> do
        ...
        case expected of
            Just expectedTyp ->
                case actual of
                    Right ((_, actualTyp), TypeCheckState _ actualSubst _ _ _ _)  -> do 
...
test_type_check_expr_14 = do
            -- 'c' >= 'd' :: Bool 
            let test = [] |= (op2 'c' GreaterEq 'd', typ @(TVar "sigma")) ~= typ @(Bool)
            executeCoreTests [test] typeCheckExpr'

test_type_check_expr_15 = do
            -- x :: a |- x >= x :: Bool
            let test = [("x", Local, forall [] $ typ @(TVar "a"))] |= 
                    (op2 (fd "x" []) GreaterEq (fd "x" []), typ @(TVar "sigma")) ~= typ @(Bool)
            executeCoreTests [test] typeCheckExpr'

test_type_check_expr_16 = do
            -- x :: a |- (-x) >= x :: Bool
            let test = [("x", Local, forall [] $ typ @(TVar "a"))] |= 
                    (op2 (op1 UnMinus (fd "x" [])) GreaterEq (fd "x" []), typ @(TVar "sigma")) ~= typ @Bool
            executeCoreTests [test] typeCheckExpr'
...
test_type_check_expr_20 = do
            -- True <= False :: ?v = Fail
            let test = [] |= (op2 True LessEq False, typ @(TVar "sigma")) ~= typ @Bool
            executeCoreTests [test] typeCheckExpr'
...
test_type_check_var_decl_8 = do
                -- [Int] x = 5 >= 3
                let test = [] |= ("x", typ @Bool) =:: op2 (iexpr 5) GreaterEq (iexpr 3)
                executeCoreTests [test] typeCheckVarDecl'

test_type_check_var_decl_9 = do
                -- x :: a |- Var y = x == x
                let test = [("x", Local, forall [] $ typ @(TVar "a"))] |= 
                            ("y", typ @Bool) =:: op2 (fd "x" []) Equal (fd "x" [])
                executeCoreTests [test] typeCheckVarDecl'
...
typeCheckFunDecl' v _ = typeCheckFunDecls (CyclicSCC [v]) >>= (\(CyclicSCC [x@(CoreFunDecl _ _ _ t _)]) -> pure (x, t))
